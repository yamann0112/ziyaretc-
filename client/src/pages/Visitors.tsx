...
import { 
  ...
  CarFront, Briefcase, User, FileText, X
} from "lucide-react";
    ...
    <div className="space-y-8 p-6 md:p-10 max-w-7xl mx-auto h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if(!open) resetForm(); }}>
          <DialogTrigger asChild>
            <button className="flex items-center gap-2 px-6 py-3 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-semibold shadow-lg shadow-primary/25 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200">
          ...
          <DialogContent className="bg-card border-border sm:max-w-xl">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold font-display">
                {editingId ? "Ziyaretçi Düzenle" : "Yeni Ziyaretçi Girişi"}
              </DialogTitle>
            </DialogHeader>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-muted-foreground ml-1">Soyad</label>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-muted-foreground ml-1">Firma</label>
                  <div className="relative">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-muted-foreground ml-1">Plaka</label>
                  <div className="relative">
                    <CarFront className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground" />
                    <input 
                      {...form.register("plate")}
                      placeholder="34 ABC 123"
                      className="w-full pl-10 pr-4 py-3 rounded-xl bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all outline-none font-mono uppercase"
                    />
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl border border-border hover:bg-white/5 transition-colors font-semibold"
                >
                  İptal
                </button>
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 transition-colors font-semibold shadow-lg shadow-primary/20"
                >
      ...
      <div className="relative max-w-md">
        ...
        <input 
          ...
          placeholder="İsim, plaka veya firma ara..."
          className="w-full pl-12 pr-4 py-3 rounded-xl bg-card border border-border focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all outline-none placeholder:text-muted-foreground/50"
        />
        {searchTerm && (
          <button 
        ...
        <AnimatePresence>
          {filteredVisitors?.map((visitor) => (
            <motion.div
              ...
              className={cn(
                "group relative bg-card border rounded-2xl p-5 shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1",
            ...
            >
              ...
              <div className="space-y-1 mb-4">
                ...
                <p className="text-xs text-muted-foreground/50 pt-1 line-clamp-1 italic">
                  {visitor.notes}
                </p>
              </div>
